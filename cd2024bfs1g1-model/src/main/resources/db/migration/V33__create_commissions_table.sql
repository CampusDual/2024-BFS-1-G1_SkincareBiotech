...
SELECT p.pro_id,
   ...
   ROUND((p.pro_sale / (1 - (cp.com_pla::numeric/100))) / (1-(cr.com_redsys::numeric/100)),2) AS SALE_PRICE,
   ROUND(COALESCE((p.pro_sale / (1 - (cp.com_pla::numeric/100))) / (1-(cr.com_redsys::numeric/100)), (p.pro_price / (1 - (cp.com_pla::numeric/100))) / (1-(cr.com_redsys::numeric/100))),2) as REAL_PRICE
  FROM products_with_offer p,
   cr,
   cp;
